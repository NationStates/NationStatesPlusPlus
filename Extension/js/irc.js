		...
		$.get("https://nationstatesplusplus.net/api/region/irc/?region=" + getVisibleRegion(), function(data) {
			if (data.irc_network) {
				...
				function showIRC(update) {
					...
					var ircURL = "https://kiwiirc.com/client/" + $("#region-irc").data("network") + ":" + $("#region-irc").data("port");
					if (isDarkTheme()) {
				...
				$("#region-irc").data("port", data.irc_port);
				$("#region-irc").data("channel", data.irc_channel);
				$("#region-irc").data("network", data.irc_network);
