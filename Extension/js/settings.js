...
(function() {
	if (getVisiblePage() == "blank" && window.location.href.indexOf("ns_settings") != -1) {
		...
		$("#save_settings").on("click", function(event) {
			...
			//Remove any save dialogues
			$("#save_confirmed").remove();
			//Create new save dialogue
			$("body").append("<div id='save_confirmed' title='Saved Successfully!'></div>");
			$("#save_confirmed").html("<p>Your settings have been saved!</p>");
			$("#save_confirmed").dialog({
				modal: true,
				buttons: {
					Ok: function() {
						$( this ).dialog( "close" );
